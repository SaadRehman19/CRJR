    ...
    >
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="bg-[#102A43] p-6 md:p-8 lg:w-[45%] flex flex-col rounded-t-[16px] md:rounded-t-[24px] lg:rounded-l-[30px] lg:rounded-tr-none">
          {/* Logo Box */}
          <div className="w-16 h-16 md:w-20 md:h-20 rounded-xl md:rounded-2xl flex items-center justify-center mb-4 md:mb-5 bg-white overflow-hidden">
            <img src={aunerPetLogo} alt="AUNER Pet Inc. Logo" className="w-20 md:w-25 h-20 md:h-25" />
          </div>

          {/* Company Name */}
          <h3 className="text-[24px] md:text-[28px] font-bold text-white mb-2 leading-tight md:leading-normal">AUNER Pet Inc.</h3>
          ...
          <div className="pt-4 border-t border-white/10 mt-auto">
            <p className="text-[13px] md:text-sm text-white/80 mb-3">Market Focus</p>
            <div className="flex gap-3 md:gap-4">
              {marketFocus.map((animal, index) => (
                <div key={index} className="flex items-center gap-2">
                  <span className="w-6 h-6 rounded-full bg-white/20 flex items-center justify-center">
                    {animal === 'Dogs' ? (
                      <svg className="w-3.5 md:w-4 h-3.5 md:h-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <span className="text-white font-medium text-[13px] md:text-sm">{animal}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="p-6 md:p-8 lg:w-[55%] flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {products.map((product, index) => (
              <div
                key={index}
                className="p-2.5 md:p-3 rounded-lg md:rounded-xl border border-[#E5E7EB]"
              >
                <div className="flex items-center gap-2 mb-1">
                  <img src={product.icon} alt="" className="w-6 md:w-8 h-6 md:h-8 shrink-0" />
                  <span className="text-[13px] md:text-sm font-semibold text-[#0F172A]">{product.title}</span>
                </div>
                <p className="text-[11px] md:text-xs text-[#4B5563] ml-8 md:ml-10">{product.description}</p>
              </div>
            ))}
          </div>
        </div>
